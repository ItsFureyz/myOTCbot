...
from bot.data.config import BOT_TIMEZONE, get_admins, GROUP_ID, TOPIC_ID
...
# Проверка карты на валидность (МИР, Visa, Mastercard российских банков)
def validate_card(card_number: str) -> bool:
    """Принимает карты 16–19 цифр: МИР (2200–2204), Visa (4), Mastercard (5/2),
    в т.ч. Сбер, Т-Банк, ВТБ, Альфа и др. Проверка алгоритмом Луна."""
    digits = "".join(ch for ch in card_number if ch.isdigit())
    if not digits.isdigit() or not (16 <= len(digits) <= 19):
        return False

    # Типичные префиксы российских / МИР карт (не жёсткий отказ по BIN, только подсказка)
    # Основная проверка — длина + Luhn
    ...
    for idx, num in enumerate(nums):
        if idx % 2 == 0:
            doubled = num * 2
            processed.append(doubled - 9 if doubled > 9 else doubled)
        else:
            processed.append(num)
    processed.append(check_digit)
    return sum(processed) % 10 == 0


def validate_yoomoney(wallet: str) -> bool:
    """Кошелёк ЮMoney: 11–20 цифр, обычно начинается с 4100."""
    digits = "".join(ch for ch in wallet if ch.isdigit())
    if not digits.isdigit():
        return False
    if not (11 <= len(digits) <= 20):
        return False
    # Классический формат счёта ЮMoney / Яндекс.Деньги
    if digits.startswith("4100"):
        return True
    # На всякий случай допускаем другие длинные цифровые идентификаторы ЮMoney
    return len(digits) >= 15


# Генерация айди сделки
def generate_deal_id() -> str:
    characters = string.ascii_lowercase + '123456789'
    return ''.join(random.choice(characters) for _ in range(10))


# Проверка является ли текст TON кошельком
def is_wallet_ton(wallet):
    for char in wallet:
        if char.isalpha():
            return True
    return False
...
# Автоматический переделает под TON форматер
def format_float_to_12_digits(number: float) -> str:
    return str(int(number) * 10**9)


def get_bank_by_country(country) -> str:
    if country == "UAH":
        return "MONO (Mono)"
    else:
        return "Sber (SberBank)"


# Проверка является ли текст float
...
def gen_id(len_id: int = 16) -> str:
    mac_address = uuid.getnode()
    time_unix = int(str(time.time_ns())[:len_id])

    # Generate mixed case random characters
    random_chars = []
    for _ in range(min(len_id, random.randint(2, 5))):
    ...
    unique_id = f"{mac_address}{time_unix}{random_str}"[:len_id]

    formatted_id = '\n'.join(wrap(unique_id, width=3))
...
def convert_date(from_time: Union[str, int], full: bool = True, second: bool = True) -> Union[str, int]:
    ...
    if str(from_time).isdigit():
        if full:
            ...
            to_time = datetime.fromtimestamp(from_time, pytz.timezone(BOT_TIMEZONE)).strftime("%d.%m.%Y в %H:%M")
        else:
...
async def send_group(bot: Bot, text: str, keyboard: InlineKeyboardMarkup = None):
    try:
        await bot.send_message(
            chat_id=GROUP_ID,
...
# Отправка сообщения всем создателям
async def send_owners(bot: Bot, text: str, keyboard: InlineKeyboardMarkup = None, not_me: int = 0):
    for admin in get_admins():
        try:
            if str(admin) != str(not_me):
                await bot.send_message(
