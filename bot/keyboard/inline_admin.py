...
def main_admin(owner: bool = False) -> InlineKeyboardMarkup:
    ...
    if owner:
        ...
        keyboard.row(
            ikb("🚫 Забанить", "admin_ban"),
            ikb("📋 Банлист", "admin_banlist"),
        )
...
def select_deals(worker: WorkerModel, is_admin: bool) -> InlineKeyboardMarkup:
    ...
    for deal in filtered_deals:
        keyboard.row(
            ikb(f"💼 Сделка {deal.deal_id} ({deal.deal_amount} {("Stars" if deal.deal_payment_method == "stars" or deal.deal_currency == "XTR" else deal.deal_currency.upper())})",
...
def banlist_markup(bans: list, page: int = 0, per_page: int = 8) -> InlineKeyboardMarkup:
    """Список забаненных с кнопками разбана."""
    builder = InlineKeyboardBuilder()
    total = len(bans)
    total_pages = max(1, (total + per_page - 1) // per_page)
    page = max(0, min(page, total_pages - 1))
    page_items = bans[page * per_page: (page + 1) * per_page]

    for ban in page_items:
        login = ban.user_login or str(ban.user_id)
        label = f"🚫 @{login} — разбанить"
        builder.row(ikb(label[:60], f"admin_unban:{ban.user_id}"))

    if total_pages > 1:
        nav = []
        if page > 0:
            nav.append(ikb("⬅️", f"admin_banlist_page:{page - 1}"))
        nav.append(ikb(f"{page + 1}/{total_pages}", "..."))
        if page < total_pages - 1:
            nav.append(ikb("➡️", f"admin_banlist_page:{page + 1}"))
        builder.row(*nav)

    builder.row(ikb("🔙 Вернуться в меню", "back_admin"))
    return builder.as_markup()
